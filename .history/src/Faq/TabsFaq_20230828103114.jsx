      ...
      {TABS.map((tab) => (
        <button
          ...
          className={`relative overflow-hidden whitespace-nowrap rounded-md border-[1px] px-3 py-1.5 text-sm font-medium transition-colors duration-500 ${
            selected === tab
              ? "border-violet-500 text-l1"
              : "border-slate-600 bg-transparent text-l5"
          }`}
          key={tab}
        >
...
const Question = ({ question, answer }) => {
  ...
  return (
    <motion.div
      ...
      className={`rounded-xl border-[1px] border-w3 px-4 transition-colors ${
        open ? "bg-d7/20" : "bg-slate-900"
      }`}
    >
      <button
