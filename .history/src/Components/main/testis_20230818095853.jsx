...
const Testis = () => {
	...
	<section className="relative z-1 max-w-[85rem] px-4 py-24 mx-auto">
		<div className="container px-5 py-12 mx-auto rounded-2xl">
		  <div className="mb-8 shadow-sm bg-d7 rounded-2xl">
			...
			<Swiper
				...
				className='w-full h-full px-2 pb-8 text-center lg:px-6 text-l1'>
			{Quotes.map((quote, index) => (
			<SwiperSlide key={index}>
				        {quote.avatar}
				<span className='px-4 pt-4 transition-all duration-500 line-clamp-6 lg:line-clamp-3 hover:line-clamp-none text-l2'>{quote.description}</span>
				<h6 className='flex flex-row items-center justify-center gap-4 mt-4 text-lg font-semibold leading-none shadow-nimble'>{quote.icon}{quote.name}</h6>
...
const Quotes = [
	{
		avatar: <Tjaden />,
		icon: <SiMinutemailer className='text-xs text-l5' />,
		description:
	...
	{
		avatar: <AvAlt />,
		icon: <SiGoogle className='text-xs text-l5' />,
	...
	{
		avatar: <Dks />,
		icon: <SiHomeadvisor className='text-xs text-l5' />,
	...
	{
		avatar: <AvAlt />,
		icon: <SiFacebook className='text-xs text-l5' />,
		description:
