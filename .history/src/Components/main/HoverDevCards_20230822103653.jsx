...
const Card = ({ title, subtitle, Icon, href }) => {
  return (
    ...
    >
      <div className="absolute inset-0 bg-gradient-to-r from-d7 to-rd1 translate-y-[100%] group-hover:translate-y-[0%] transition-transform duration-300" />

      <Icon className="absolute z-10 transition-transform duration-300 -top-1 -right-1 text-7xl text-b3 group-hover:text-r group-hover:rotate-12" />
      <Icon className="relative z-10 mb-2 text-2xl transition-colors duration-300 text-r group-hover:text-yw" />
      <h3 className="relative z-10 pt-2 text-xl font-medium duration-300 text-l1 group-hover:text-white">
        {title}
      </h3>
      <p className="relative z-10 pt-2 text-xs font-thin duration-300 text-l3 group-hover:text-l1">
        {subtitle}
      </p>
    </a>
  );
