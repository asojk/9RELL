...
const MenuItem = ({ to, title, icon, onClick }) => (
  <Link to={to} onClick={onClick}>
    <button className="flex items-center px-2 py-2 transition-colors duration-150 rounded-lg cursor-pointer lg:px-6 group group-hover:text-r">
      <span className="col-span-1 pr-4 group">
        <motion.span variants={iconVariants}>{icon}</motion.span></span>
      <span className="text-sm font-medium leading-none md:text-base text-l1 group">
        {title}
      </span>
    </button>
  </Link>
