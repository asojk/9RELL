    ...
    <AnimatePresence>
      {isOpen && (
        ...
        <nav className="flex flex-col flex-wrap w-full p-6">
          <div className="w-full pl-3 text-sm text-l4">
            Services</div>
          ...
          <div className="w-full pl-4 text-sm text-w4">
            Information</div>
...
const MenuItem = ({to, title, icon}) => (
  <Link to={to}>
    <button className="flex items-center px-6 py-2 cursor-pointer md:py-4 rounded-xl">
      ...
      <span className="text-sm font-medium leading-none md:text-base text-l1 md:text-lg">{title}</span>
    </button>
  </Link>
);
