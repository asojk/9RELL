...
<AnimatePresence> {isOpen && (
  <motion.div ref={node} variants={sidebarVariants} initial="closed" animate="open" exit="closed"
    className="fixed top-0 left-0 z-50 h-screen py-12 overflow-auto text-base border-r-2 w-42 lg:max-w-[20rem] lg:w-[25vw] bg-gradient-to-r from-d7 to-d6/95 via-d7 rounded-r-lg text-l1 border-b3">
  <nav className="flex flex-col flex-wrap justify-center w-full px-6 lg:px-8">
    ...
    <div className="relative right-0 flex items-baseline justify-center w-full z-9 bottom-4">
...
const MenuItem = ({ to, title, icon, onClick }) => (
  <Link to={to} onClick={onClick}>
    <button className="flex items-center px-2 py-2 transition-colors duration-150 rounded-lg cursor-pointer lg:px-6">
      <span className="col-span-1 pr-4">
        <motion.span variants={iconVariants}>{icon}</motion.span></span>
      <span className="text-sm font-medium leading-none md:text-base text-l1">
        {title}
      </span>
    </button>
  </Link>
