		...
		<AnimatePresence>
			...
			{isOpen && (
				<motion.div
					...
					animate='open'
					exit='closed'
					className='fixed top-0 left-0 h-screen py-4 overflow-auto w-content '>
					<nav className='grid w-full px-6 h-content place-content-center lg:px-8'>
						<motion.div variants={menuVariants}>
							<ul className='flex flex-col h-fit justify-items-center gap-4 bg-gradient-to-l from-d7 to-d8 via-d8 rounded-r-lg border-b3 z-[99] border-r-2 '>
								...
								<div className='display-block md:hidden'>
									<div className='w-full pt-4 pb-2 pl-2 text-sm text-w4'>Contact</div>
									<Cbuttons />
								</div>
								<Link to='/' onClick={onClose}>
									<span className='grid place-content-center border-[1px] mx-8 mt-4 lg:mt-12 rounded-md shadow-nimble border-b3 hover:border-l5'>
										<a className='inline-flex items-center p-3 text-md gap-x-2'>
											<FiHome className='w-6 h-6 lg:w-8 lg:h-8' />
											Home
										</a>
									</span>
								</Link>
...
const MenuItem = ({ to, title, icon, onClick }) => (
	<Link to={to} onClick={onClick}>
		<button className='flex items-center px-2 py-2 transition-colors duration-150 cursor-pointer lg:px-6 group'>
			...
			<span className='text-lg font-thin leading-relaxed text-l1 group-hover:text-w4 group-hover:scale-110'>
				{title}
			</span>
		</button>
	</Link>
