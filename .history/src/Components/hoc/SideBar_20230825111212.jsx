	...
	<AnimatePresence>
		...
		{isOpen && (
			<motion.div
				...
				animate='open'
				exit='closed'
				className='fixed top-0 left-0 h-screen py-8 overflow-auto text-lg border-r-2 w-42 md:text-xl lg:text-2xl md:max-w-[20rem] md:w-[45vw] bg-gradient-to-r from-d7 to-d6/95 via-d7 rounded-r-lg text-l1 border-b3 z-[99]'>
				<div className='pb-8 display-block md:hidden'>
					<Cbuttons />
				</div>
				<nav className='flex flex-col flex-wrap justify-center w-full px-6 lg:px-8'>
					...
					<div className='flex items-baseline justify-center w-full pt-8'>
						...
						<Link to='/' onClick={onClose}>
							<button type='button' className='p-3 md:mt-6 btn border-b3 shadow-nimble text-l1'>
								<FiHome />
...
const MenuItem = ({ to, title, icon, onClick }) => (
  <Link to={to} onClick={onClick}>
    <button className="flex items-center px-2 py-2 transition-colors duration-150 rounded-lg cursor-pointer lg:px-6 group">
      ...
      <span className="text-lg font-thin leading-none md:text-xl lg:text-2xl text-l1 group-hover:text-w4 group-hover:scale-110">
        {title}
      </span>
    </button>
  </Link>
