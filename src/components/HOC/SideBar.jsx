							...
							<ul className='flex flex-col h-fit justify-items-center gap-4 z-[99] '>
								...
								<Link to='/' onClick={onClose}>
									<span className='grid place-content-center border-[1px] mx-8 mt-4 lg:mt-12 rounded-md shadow-nimble border-b3 hover:border-l5'>
										<a className='inline-flex items-center p-3 sm:text-sm md:text-lg lg:text-xl gap-x-2 group'>
											<FiHome className='sm:text-sm md:text-lg lg:text-xl group-hover:text-white' />
											Home
										</a>
									</span>
								</Link>
...
const MenuItem = ({ to, title, icon, onClick }) => (
	<Link to={to} onClick={onClick}>
		<button className='flex items-center px-2 py-2 transition-colors duration-150 cursor-pointer lg:px-6 group'>
			...
			<span className='font-thin leading-relaxed sm:text-sm md:text-lg lg:text-xl text-l1 group-hover:text-w4 group-hover:scale-110'>
				{title}
			</span>
		</button>
	</Link>
